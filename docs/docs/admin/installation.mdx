...
| `PUBLIC_URL_DYNAMIC`           | `false`                 | If set to `true`, derive the public URL from the `X-Forwarded-Host` header instead of the static `PUBLIC_URL`. Use this when one Anubis instance protects multiple domains via forwardAuth; the challenge path (`/.within.website`) must be routed to Anubis on every protected domain. Requires `REDIRECT_DOMAINS` to be set, otherwise Anubis could be used as an open redirect. Overrides `PUBLIC_URL` if both are set.                                                                                                                     |
